...
int main(int argc, char** argv)
{
    ...
    if (idxStartData > 0)
    {
        ...
        if (pData)
        {
			...
			stringstream ss;
			ss << FileName;
			string output_filename = ss.str() + ".vtk";
			...
			myfile << "DIMENSIONS " << xDim << " " << yDim << " " << zDim << endl;
			myfile << "SPACING 1 1 1" << endl;
			myfile << "ORIGIN 0 0 0" << endl;
			myfile << "POINT_DATA " << xDim * yDim * zDim<< endl;
            ...
            for(int k=0;k<zDim;k++)
            {
                ...
                for(int j=0;j<yDim;j++)
                {
                    for(int i=0;i<xDim;i++)
                    {
						...
						float w = pData[Idx * NumComponents + 2];
						
						myfile << sqrt(u*u + v*v + w*w) << "\n";
